...
// Componente auxiliar para os cards de métricas (estilizado para tema escuro)
const MetricCard = ({ title, value, icon: Icon, className = "" }) => (
  <div className={`p-4 bg-gray-800 rounded-lg shadow-md flex items-center justify-between ${className}`}>
    <div>
      <p className="text-sm font-medium text-gray-300">{title}</p>
      <p className="text-2xl font-bold text-gray-50 mt-1">{value}</p>
    </div>
    {Icon && <Icon className="w-8 h-8 text-indigo-300 opacity-90" />}
  </div>
);
...
const BarChart = ({ data, title, dataKey, valueKey, color = "bg-indigo-500" }) => {
  ...
  return (
    <div className="bg-gray-800 p-6 rounded-lg shadow-md">
      <h3 className="text-lg font-semibold text-gray-50 mb-4">{title}</h3>
...
export default function DashboardJogadoras() {
  ...
  if (loading && !dataFetched) {
    return (
      <div className="flex justify-center items-center h-64">
        ...
        <span className="ml-2 text-lg text-gray-50">Carregando dados do dashboard...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-6 bg-red-900 border border-red-700 text-red-100 rounded-lg">
  ...
  return (
    <div className="p-8 min-h-screen text-gray-50 bg-[rgba(0,0,0,0.3)] border border-gray-700 rounded-lg mt-25">
      <h1 className="text-3xl font-bold text-gray-50 mb-8 text-center">Dashboard de Jogadoras</h1>
